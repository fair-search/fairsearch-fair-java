...
public class FairTests {
    ...
    private static final double OFFSET = 0.0001; // result tolerance for FairTests
    ...
    public void testIsFair(int k, double p, double alpha, TopDocs ranking) {
        ...
        assertEquals(true, fair.isFair(ranking));
    }
    ...
    public void testCreateUnadjustedMTable(int k, double p, double alpha, int[] expected){
        Fair fair = new Fair(k, p, alpha);
        assertArrayEquals(expected, fair.createUnadjustedMTable());
    }
    ...
    public void testCreateAdjustedMTable(int k, double p, double alpha, int[] expected){
        Fair fair = new Fair(k, p, alpha);
        assertArrayEquals(expected, fair.createAdjustedMTable());
    }
    ...
    public void testAdjustAlpha(int k, double p, double alpha, double expected) {
        ...
        double actual = fair.adjustAlpha();
    ...
    public void testComputeFailureProbability(int k, double p, double alpha, double expected) {
        ...
        double actual = fair.computeFailureProbability(adjustedMTable);
    ...
    public void testMTableGeneration(int k, double p, double alpha) {
        ...
        Fair fairAdjusted = new Fair(k, p, alphaAdjusted);
        int[] unadjustedMTable = fairAdjusted.createUnadjustedMTable();
    ...
    public void testReRank(int k, double p, double alpha, TopDocs ranking) {
        ...
        TopDocs reRanked = fair.reRank(ranking);

        // input should not be fair
        assertEquals(false, fair.isFair(ranking));
        ...
        // output should not be fair
        assertEquals(true, fair.isFair(reRanked));
    }
}
